...
def transcribe(
    ...
    def filter_intervals_for_chunk(
            intervals: List[Tuple[int, int]],
            chunk_start_ms: float,
            chunk_end_ms: float
    ) -> List[Tuple[int, int]]:
        """
        Ermittelt alle Intervalle aus 'intervals', die in den Bereich [chunk_start_ms, chunk_end_ms]
        hineinfallen, und schneidet sie ggf. passend zu. Offset wird so berechnet,
        dass das Chunk bei 0 ms startet.
        """
        relevant = []
        for (start_ms, end_ms) in intervals:
            # Prüfen, ob irgendeine Überschneidung mit dem aktuellen Chunk vorliegt
            if end_ms < chunk_start_ms or start_ms > chunk_end_ms:
                continue  # kein Overlap

            # Clipping auf den Chunk
            clipped_start = max(start_ms, chunk_start_ms)
            clipped_end = min(end_ms, chunk_end_ms)

            # Offset so, dass im lokalen Chunk bei 0ms begonnen wird
            clipped_start -= chunk_start_ms
            clipped_end -= chunk_start_ms

            relevant.append((int(clipped_start), int(clipped_end)))
        return relevant

    def decode_with_fallback(segment: torch.Tensor,
                             transcription_segments: Optional[List[Tuple[int, int]]]) -> DecodingResult:
        """
        Decode a segment of audio with fallback options for temperature and thresholds.
        If transcription_segments is provided, it will be used as a call parameter for model.decode

        :param segment: The audio segment to decode
        :param transcription_segments: Optional list of tuples representing the start and end timestamps of segments, in ms
        """
        temperatures = (
    ...
    with tqdm.tqdm(
        ...
        while clip_idx < len(seek_clips):
            ...
            # if transcription_intervals is provided, calculate the intervals for relevant chunk (time_offset + segment_duration) and move them accordingly
            if transcription_intervals:
                chunk_start_ms = time_offset * 1000.0
                chunk_end_ms = (time_offset + segment_duration) * 1000.0
                relevant_intervals = filter_intervals_for_chunk(
            ...
            result: DecodingResult = decode_with_fallback(mel_segment, relevant_intervals)
...
def cli():
    ...
    parser.add_argument("--carry_initial_prompt", type=str2bool, default=False,
                        help="if True, prepend initial_prompt to every internal decode() call. May reduce the effectiveness of condition_on_previous_text")
